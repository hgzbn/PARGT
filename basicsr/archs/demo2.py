...
class ATD_CA(nn.Module):
    r"""
    Optimized ATD_CA for local feature extraction.

    Args:
        dim (int): Number of input channels.
        num_heads (int): Number of attention heads.
        input_resolution (tuple[int]): Input resolution.
        reducted_dim (int, optional): Reduced dimension number for query and key matrix. Default: 8
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
    """

    def __init__(self, dim, num_heads=4, input_resolution=(64, 64), reducted_dim=8, qkv_bias=True):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        # Linear layers for Q, K, V in multi-head attention
        self.wq = nn.Linear(dim, reducted_dim * num_heads, bias=qkv_bias)
        self.wk = nn.Linear(dim, reducted_dim * num_heads, bias=qkv_bias)
        self.wv = nn.Linear(dim, dim, bias=qkv_bias)  # 保持 V 的输出维度为 dim

        # Depthwise convolution to enhance local feature extraction
        self.depthwise_conv = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim)

        # Scaling factor and softmax
        self.scale = nn.Parameter(torch.ones(num_heads) * 0.5, requires_grad=True)
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, x, x_size):
        h, w = x_size
        b, n, c = x.shape
        head_dim = self.reducted_dim

        # Q, K, V transformations with multi-head support
        q = self.wq(x).reshape(b, n, self.num_heads, head_dim).permute(0, 2, 1, 3)  # (b, num_heads, n, head_dim)
        k = self.wk(x).reshape(b, n, self.num_heads, head_dim).permute(0, 2, 1, 3)  # (b, num_heads, n, head_dim)
        v = self.wv(x).reshape(b, n, self.num_heads, c // self.num_heads).permute(0, 2, 1, 3)  # (b, num_heads, n, head_dim)

        # Attention scores
        attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1))
        attn = attn * self.scale.view(1, self.num_heads, 1, 1)  # Scale per head
        attn = self.softmax(attn)  # (b, num_heads, n, n)

        # Weighted sum of values
        x = (attn @ v).permute(0, 2, 1, 3).reshape(b, n, c)  # (b, n, c)

        # Reshape for convolution and apply depthwise convolution for local feature enhancement
        x = x.reshape(b, h, w, c).permute(0, 3, 1, 2)  # Reshape to (b, c, h, w)
        x = self.depthwise_conv(x).reshape(b, c, -1).permute(0, 2, 1)  # Back to (b, n, c)
...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, dim, reduction=4):
        ...
        # 两个全连接层，用于生成注意力权重
        self.fc1 = nn.Conv2d(dim, dim // reduction, kernel_size=1, bias=False)
        self.relu = nn.ReLU()
        self.fc2 = nn.Conv2d(dim // reduction, dim, kernel_size=1, bias=False)
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x, H, W):
        # Split
        x1, x2 = x.chunk(2, dim=-1)  # Ensure x1 and x2 split correctly
        B, N, C = x.shape

        # Check that the dimension of x2 is C//2
        x2 = self.conv(self.norm(x2).transpose(1, 2).contiguous().view(B, C // 2, H, W)).flatten(2).transpose(-1,
                                                                                                              -2).contiguous()
...
class SGFN(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.act = act_layer()
        self.sg = SpatialGate(hidden_features // 2)  # Ensure correct initialization
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        # Define FusedConv module
        self.fused_conv = FusedConv(num_feat=dim, expand_size=4, attn_ratio=4)

        # Define L_SA and ATD_CA
        if idx % 2 == 0:
            self.attn_l_sa = L_SA(
        ...
        self.ffn = SGFN(in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim,
    ...
    def forward(self, x, x_size):
        ...
        # Apply FusedConv before attention
        x = self.fused_conv(x, x_size, None, None)
        ...
        if self.idx % 2 == 0:
            ...
            x_atd = self.atd_ca(self.norm1(x), x_size)  # 不再传入 td
            x = x + self.drop_path(x_l_sa + x_atd)
        else:
            # Apply RG_SA
            x = x + self.drop_path(self.attn_rg_sa(self.norm1(x), H, W))
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop,
                attn_drop=attn_drop,
                drop_path=drop_paths[i] if drop_paths is not None else 0.,
                act_layer=act_layer,
