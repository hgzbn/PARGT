...
class ATD_CA(nn.Module):
    def __init__(self, dim, input_resolution, num_tokens=32, reducted_dim=8, qkv_bias=True):
        super().__init__()
        self.dim = dim
        ...
        self.wv = nn.Linear(dim, dim, bias=qkv_bias)  # 确保 V 的维度为 dim，保持与 L_SA 一致

        self.scale = nn.Parameter(torch.ones([1]) * 0.5, requires_grad=True)
    ...
    def forward(self, x, td, x_size):
        ...
        scaled_factor = torch.clamp(self.scale, 0, 1)
        attn = attn * (1 + scaled_factor * np.log(self.num_tokens))
        attn = self.softmax(attn)

        x = (attn @ v).reshape(b, n, -1)  # 动态调整最后一个维度以匹配 size
