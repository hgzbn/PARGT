...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        if idx % 2 == 0:
            ...
            # Use the updated CSAM with Channel and Spatial gates
            self.csam = CSAM(dim, reduction_ratio=16)
        else:
            self.attn_g_sa = G_SA(
    ...
    def forward(self, x, x_size):
        ...
        if self.idx % 2 == 0:
            # Apply SWPA and the modified CSAM in parallel
            x_swpa = self.attn_swpa(self.norm1(x), H, W)

            # Reshape x to (B, C, H, W) before passing to CSAM
            x_reshaped = rearrange(self.norm1(x), "b (h w) c -> b c h w", h=H, w=W)
            x_atd = self.csam(x_reshaped)
