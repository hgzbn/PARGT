...
class SpatialGate(nn.Module):
    """基于内容的动态分割，通过通道注意力分配通道."""

    def __init__(self, dim):
        super(SpatialGate, self).__init__()
        self.norm = nn.LayerNorm(dim)
        self.conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim)  # DW Conv
        self.channel_attention = ChannelAttention(dim)  # 引入通道注意力
    ...
    def forward(self, x, H, W):
        ...
        # 按权重分割通道
        x = x * attention
        x1, x2 = x.chunk(2, dim=1)  # 按照通道分成两部分

        x2 = self.conv(self.norm(x2).flatten(2).transpose(1, 2).contiguous().view(B, C // 2, H, W)).flatten(
            2).transpose(-1, -2).contiguous()

        return x1 * x2
class SGFN(nn.Module):
    """带有动态分割的Spatial-Gate Feed-Forward Network."""
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
