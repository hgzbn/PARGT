...
class CAB(nn.Module):
    ...
    def __init__(self, num_feat, compress_ratio=3, squeeze_factor=30):
        ...
        # self.cab = NonLocalSparseAttention(channels=num_feat, chunk_size=36, n_hashes=4, reduction=4, res_scale=1)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
                 ...
                 num_tokens=64, reducted_dim=10, kernel_size=5, compress_ratio=3, squeeze_factor=30):
        super().__init__()
        ...
        if idx % 2 == 0:
            # Apply CAB and RG_SA in parallel
            self.attn_rg_sa = RG_SA(
                dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                proj_drop=drop, c_ratio=c_ratio
            )

            self.cab = CAB(dim, compress_ratio=compress_ratio, squeeze_factor=squeeze_factor)
    ...
    def forward(self, x, x_size, td=None):
        ...
        if self.idx % 2 == 0:
            ...
            # Reshape and permute x for CAB
            B, N, C = x.shape
            x_reshaped = x.view(B, H, W, C).permute(0, 3, 1, 2)  # B, C, H, W
            x_cab = self.cab(x_reshaped)
            x_cab = x_cab.view(B, C, H * W).permute(0, 2, 1)  # B, N, C

            x = x + self.drop_path(x_rg_sa + x_cab)
        else:
