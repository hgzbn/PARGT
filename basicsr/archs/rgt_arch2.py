...
class LongSkipConnection(nn.Module):
    def __init__(self, dim):
        ...
        self.conv = nn.Conv2d(dim, dim, kernel_size=1)  # 一个简单的卷积操作，可以根据需要更改
...
#         return x
class ResidualGroup(nn.Module):
    def __init__(self, dim, reso, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
                 attn_drop=0., drop_paths=None, act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 depth=2, use_chk=False, resi_connection='1conv', rs_id  =0,
                 split_size=[8, 8], c_ratio=0.5, long_skip_connection=True):
        super().__init__()
        self.use_chk = use_chk
        self.reso = reso
        ...
        self.long_skip_connection = LongSkipConnection(dim) if long_skip_connection else None
    ...
    def forward(self, x, x_size):
        ...
        if self.long_skip_connection is not None:
            x = self.long_skip_connection(x, x_size)
