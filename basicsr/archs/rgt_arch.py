...
class ShuffleAttention(nn.Module):
    def __init__(self, channel, reduction=16, groups=2):
        ...
        # Channel attention weights and biases
    ...
    def forward(self, x):
        ...
        if c % (2 * self.groups) != 0:
            raise ValueError(f"Channel size {c} must be divisible by 2 * groups {2 * self.groups}.")
        ...
        # Spatial attention
        x_spatial = F.layer_norm(x_1, x_1.size()[1:])
        ...
        out = out.view(b, c, h, w)  # 恢复原始通道数
        return out

class CSSAM(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=16):
        super(CSSAM, self).__init__()
        self.ChannelGate = ChannelGate(gate_channels, reduction_ratio)
        self.SpatialGate = SpatialGate1()
        self.ShuffleAttention = ShuffleAttention(gate_channels, reduction=reduction_ratio, groups=2)
    ...
    def forward(self, x):
        ...
        # Integrate ShuffleAttention
        x_out = self.ShuffleAttention(x_out)
        return x_out

class SWPA(nn.Module):
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        if idx % 2 == 0:
            ...
            # Use the updated CSSAM with Channel and Spatial gates
            self.cssam = CSSAM(dim, reduction_ratio=16)
        else:
            self.attn_g_sa = G_SA(
    ...
    def forward(self, x, x_size):
        ...
        if self.idx % 2 == 0:
            # Apply SWPA and the modified CSSAM in parallel
            x_swpa = self.attn_swpa(self.norm1(x), H, W)

            # Reshape x to (B, C, H, W) before passing to CSSAM
            x_reshaped = rearrange(self.norm1(x), "b (h w) c -> b c h w", h=H, w=W)
            x_atd = self.cssam(x_reshaped)

            # Reshape x_atd back to (B, H*W, C)
            x_atd = rearrange(x_atd, "b c h w -> b (h w) c", h=H, w=W)

            x = x + self.drop_path(x_swpa + x_atd)
        else:
            # Apply G_SA
            x = x + self.drop_path(self.attn_g_sa(self.norm1(x), H, W))

        # Apply second LayerNorm
        x = self.norm2(x)

        # Apply FusedConv before SGFN
        x = self.fused_conv(x, x_size, None, None)

        # Apply SGFN
        x = x + self.drop_path(self.ffn(x, H, W))
