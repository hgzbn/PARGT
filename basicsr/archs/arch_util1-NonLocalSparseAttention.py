...
class RGT(nn.Module):
    def __init__(self,
        ...
        self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1).to(self.device)
        ...
        self.nlsa1 = NonLocalSparseAttention(channels=embed_dim, chunk_size=36, n_hashes=4, reduction=4,
                                             res_scale=1).to(self.device)
        self.nlsa2 = NonLocalSparseAttention(channels=embed_dim, chunk_size=36, n_hashes=4, reduction=4,
                                             res_scale=1).to(self.device)
        ...
        self.norm = norm_layer(curr_dim).to(self.device)
        self.nlsa3 = NonLocalSparseAttention(channels=embed_dim, chunk_size=36, n_hashes=4, reduction=4,
                                             res_scale=1).to(self.device)
        self.nlsa4 = NonLocalSparseAttention(channels=embed_dim, chunk_size=36, n_hashes=4, reduction=4,
                                             res_scale=1).to(self.device)

        # Build the last conv layer in deep feature extraction
        if resi_connection == '1conv':
            self.conv_after_body = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1).to(self.device)
        elif resi_connection == '3conv':
        ...
        self.upsample = Upsample(upscale, num_feat).to(self.device)
        self.conv_last = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1).to(self.device)
    ...
    def forward_features(self, x):
        _, _, H, W = x.shape
        x_size = [H, W]
        x = x.to(self.device)  # Ensure x is on the same device as the model
        x = self.before_RG(x)
    ...
    def forward(self, x):
        ...
        x = self.forward_features(nlsa_st)
